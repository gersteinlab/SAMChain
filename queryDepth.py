...
Usage: $ python queryDepth.py <chainName> <readLocation> -ml/--multichainLoc <multichainLoc>
Example: python queryDepth.py chain1 chr1:1005-2005
modified by CMB on 10/22/2019
'''
...
def queryAllData(multichainLoc, readLoc,  chainName, dr, key, reference):
    ...
    if len(loc) > 1: #location queried was a RANGE
        for pos in range(int(loc[0]), int(loc[1])+1):
            pos_query_dict[int(pos)] = 0
    else:
        pos_query_dict[int(loc[0])] = 0

    for name in key: #loop through the keys that correspond to the queried location
        ...
        for result in matches:
            ...
            for pos in range(int(pos), counter):
                if pos in pos_query_dict:
                    if pos not in pos_exceptions:
                        pos_query_dict[pos] += 1
