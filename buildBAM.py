...
def buildDataFrame(data):
	columns = ['readID', 'POS', 'endPOS', 'cigar','data']
	array = []
	for item in data:
		#convert the keys to ints here
		readID = item["keys"][0][7:]
		pos = int(item["keys"][1][4:])
		endPos = int(item["keys"][3][7:])
                cigar = item["keys"][4][6:]
                cig = cigparse(cigar)
		array.append([readID, pos, endPos, cig, item["data"]])
